...
@section('content')
    <form action="{{ route('purchase.store') }}" method="post">
        ...
        <section>
            @php
                $purchaseNumber = config('site.purchase_prefix') . date('y') . str_pad(config('site.purchase_number'), 7, '0', STR_PAD_LEFT);
            @endphp
            @if ($errors->any())
                ...
                <div class="row">
                    <div class="col-sm-12 col-md-4 mt-3 justify-content-between">
                        ...
                        <input class="form-control form-control-lg" name="reference" type="text" readonly
                            value="{{ $purchaseNumber }}" aria-label="default input example">
                    </div>
                    <div class="col-sm-12 col-md-4 mt-3 justify-content-between">
                    ...
                    <div class="col-sm-12 col-md-4 mt-3 justify-content-between">
                        ...
                        <select class="form-select form-select-lg" id="discount_type_invoice" name="discount_type"
                            ...
                            <option value="amount" {{ old('discount_type') == 'amount' ? 'selected' : '' }}>مبلغ</option>
                            <option value="percent" {{ old('discount_type') == 'percent' ? 'selected' : '' }}>نسبه مئوية
                            </option>
                        </select>
                    </div>
                    <div class="col-sm-12 col-md-4 mt-3 justify-content-between">
                        <label class="">المخزن</label>
                        <select class="form-select select2 form-select-lg " name="store_id"
                            aria-label=".form-select-lg example">
                    ...
                    <div class="col-sm-12 col-md-4 mt-3  justify-content-between">
                        <label class="">المورد</label>
                        <select class="form-select form-select-lg select2 " name="supplier_id"
                            aria-label=".form-select-lg example">
                        ...
                        @if (old('product_id'))
                            @foreach (old('product_id') as $key => $value)
                                <tr id="r{{ $key + 1 }}">
                                    ...
                                    <td>
                                        <p class="total">
                                            {{ old('discount_type_p')[$key] == 'amount' ?number_format( old('purchase_price')[$key] * old('quantity')[$key] - old('discount_p')[$key] * old('quantity')[$key] , 2): number_format((old('purchase_price')[$key] * old('quantity')[$key]) - (old('purchase_price')[$key] * old('discount_p')[$key] * old('quantity')[$key]) / 100, 2) }}
                                        </p>
                                    </td>
